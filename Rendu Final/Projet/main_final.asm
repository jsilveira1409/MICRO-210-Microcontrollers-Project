...
.equ	bufferLen		= 12			;taille du buffer		
.equ	SRAM_flag		= 0
.equ	EEPROM_START	= 0				
.equ	eepromLen		= 0x00b4		;taille de la mémoire allouée au stockage

; ======================= memory management ======================================
.dseg
.org	SRAM_START						;début du buffer dans la sram
...
overflow0:								;timer overflow (activation du flag 
		in		_sreg, SREG				;pour stockage dans la SRAM des
		ori		b3, (1<<SRAM_flag)		;trois mesures des capteurs)
		out		SREG, _sreg
		reti
...
reset:
		...
		OUTI		ASSR,  (1<<AS0)				
		OUTI		TCCR0,6						;overflow toutes les 2 secondes
...
.include "libPerso/per_eeprom.asm"				;libs du cours modifiés par nous pour que les ports et 
.include "libPerso/per_encoder.asm"				;les registres ne se superposent pas
.include "libPerso/per_wire1.asm"				;librairies personnelles
...
main:	
		LINEAR_MENU			a0,0,3				;limite a0 à 0-1-2-3 pour le menu (menu non-cyclique)
		PRINTF			LCD
.db		CR, CR,FHEX,a,0							
		rcall			menui
.db		"Temperature |Humidity    |Light       |Upload      ",0	;choix dans le menu	(affichage)
		WAIT_MS			1
		rcall			encoder					;vérifie l'etat de l'encodeur angulaire pour les changements
		...
		rcall			store					;stocke dans la SRAM et vérifie si elle est pleine 
		rjmp			main					;pour ensuite stocker dans l'eeprom
...
.db	"temp  ",FFRAC2+FSIGN,b,4,$42," C",CR,0
		rcall		encoder				;vérifie l'état de l'encodeur au cas où il a été appuyé
...
.db	"hum   ",FFRAC2+FSIGN,b,4,$42," %",CR,0
		rcall		encoder				;vérifie l'etat de l'encodeur au cas où il a été appuyé
		brts		come_back			;revient si c'est le cas
		sbrc		b3, SRAM_flag		;flag pour stockage active et on stocke les 3 valeurs dans la sram
...
.db	"Uploading...",0
	...
	loop_upload:
		...
		cpi			b1, 0x00
		brne		PC+3
		cpi			b0, 0x00					;check pour envoyer les valeurs qui sont modifiés
		breq		PC+4

		rcall		putc						;send instruction b0 via uart
		mov			b0, b1						;-> b0 b1 et non pas b1 b0 (big endian)
		rcall		putc
...
set_eeprom:									
		ldi			b0, 0x00				;remplir la eeprom de zeros à l'initialisation

		loop_set:
		rcall		eeprom_store  
		adiw		xl,1					;incrémentation de l'adresse de la eeprom (->word)
		cpi			xl, low(eepromLen)		;xl max
		brne		loop_set
		cpi			xh, high(eepromLen)		;xh max
		brne		loop_set
		...
		ldi			xh, high(EEPROM_START)	;pour réinitialiser le pointeur après avoir mis la eeprom à 0x00
		ret
